...
import { getReturnPolicy } from "../../lib/repos/return_policy";

export async function generateMetadata() {
  return {
    title: "Dubblin Return Policy",
    description: "Dubblin Return Policy",
    tags: "Dubblin Return Policy",
  };
}

const ReturnPolicyPage: React.FC = async () => {
  const data = await getReturnPolicy();
  const description = data?.data?.ReturnPolicy[0]?.description;
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          {description ? (
            ...
            <div className="p-6 sm:p-8 text-center text-gray-800 font-Outfit text-base sm:text-lg">
              <p>No Return Policy are available at this time.</p>
            </div>
          )}
        </div>
      </div>
